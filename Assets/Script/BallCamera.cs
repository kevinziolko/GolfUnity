...
public class BallCamera : MonoBehaviour
{
    ...
    void LateUpdate() // s'exécute après Update (on connaît la position de la cam)
    {
        // mises en place des directives
                ...
                {
                    x += Input.GetTouch(0).deltaPosition.x * 0.1f; // La position du doigt sur l'axe des x est calculée
                }
#endif
        ...
        {
            transform.position = new Vector3(transform.position.x, 3.5f, transform.position.z); // mon niveau était un peu en dessous
